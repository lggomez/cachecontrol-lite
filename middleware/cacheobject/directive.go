...
func (cd *ResponseCacheDirectives) BuildResponseHeader() string {
	...
	if cd.Immutable {
		cd.MaxAge = DeltaSeconds(defaultMaxAge.Seconds())
	}
	...
	if cd.MaxAge != 0 {
		headerTokens = append(headerTokens, fmt.Sprintf("max-age=%d", cd.MaxAge))
	}

	if cd.SMaxAge != 0 {
		headerTokens = append(headerTokens, fmt.Sprintf("s-maxage=%d", cd.SMaxAge))
	}

	if cd.StaleIfError != 0 {
		headerTokens = append(headerTokens, fmt.Sprintf("stale-if-error=%d", cd.StaleIfError))
	}

	if cd.StaleWhileRevalidate != 0 {
		headerTokens = append(headerTokens, fmt.Sprintf("stale-while-revalidate=%d", cd.StaleWhileRevalidate))
	}
